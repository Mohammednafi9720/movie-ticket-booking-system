...
function displayMovies(list = movies) {
    ...
    list.forEach(m => {
        ...
        <div class="movie-card">
            ...
            <p>⭐ ${m.rating}</p>
            <h4>₹${m.price}</h4>
            <button onclick="bookMovie(${m.id})">Book Now</button>
        </div>`;
    });
}
displayMovies();
...
function searchMovie() {
    const val = document.getElementById("searchMovie").value.toLowerCase();
...
function bookMovie(id) {
    ...
    document.getElementById("movieName").value = selectedMovie.title;
...
document.getElementById("bookingForm").addEventListener("submit", function(e){
    ...
    if (selectedSeats.length !== seatCount) {
        alert("Select correct seats");
        return;
    }

    const bookingId = "BK" + Math.floor(Math.random()*99999);

    bookingCounter++;
    document.getElementById("bookingCount").innerText = bookingCounter;
    ...
    alert("Booking Successful: " + bookingId);
...
function deleteFirstBooking() {
    head = head?.next;
    bookingCounter = Math.max(0, bookingCounter - 1);
    document.getElementById("bookingCount").innerText = bookingCounter;

    alert("First booking deleted");
}